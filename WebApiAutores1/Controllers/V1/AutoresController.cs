...
namespace WebApiAutores1.Controllers.V1
{
    ...
    public class AutoresController : ControllerBase
    {
        ...
        [AllowAnonymous]
        [ServiceFilter(typeof(HATEOASAutorFilterAtribute))]
        public async Task<ActionResult<List<AutorDTO>>> Get([FromQuery] PaginacionDTO paginacionDTO)
        {
            var queryable = context.Autores.AsQueryable();
            await HttpContext.InsertarParametrosPaginacionEnCabecera(queryable);
            var autores = await queryable.OrderBy(autor => autor.Nombre).Pagina(paginacionDTO).ToListAsync();
        ...
        public async Task<ActionResult<AutorDTOConLibros>> Get(int id)
        {
            var autor = await context.Autores
                ...
                .ThenInclude(autorLibrosDB => autorLibrosDB.Libro)
